...
kind: Role
spec:
    description: Adversarial periodic health-check workflow for detecting and restoring specification drift across requirements, design, validation, documentation, tests, and implementation.
    provider:
        type: codex
        model: gpt-5.4
        reasoning: xhigh
...
PromptKit parameter `context`:

{{ .Input }}

PromptKit parameter `design_path` — Path to the existing design document:

{{ index .Params "design_path" }}

PromptKit parameter `requirements_path` — Path to the existing requirements document:

{{ index .Params "requirements_path" }}

PromptKit parameter `validation_path` — Path to the existing validation plan:

{{ index .Params "validation_path" }}
...
- Requirements: Discover in the repository, including Beads story requirements and acceptance
- Design: Discover in the repository, including Beads story design and architecture docs
- Validation: Discover in the repository, including tests, CI, verification evidence, and Beads child-task evidence

**Implementation**: /home/bigboss/Projects/knowl

**Verification**: /home/bigboss/Projects/knowl

**Focus Areas**: Specification drift between Beads requirements/design/acceptance, repository docs, tests, verification, and Go implementation; prioritize recent changes and public contracts

**Additional Context**:
The target repository is Knowl, a Go local-first Markdown knowledge service.
The semantic baseline may be stored in Beads issues and their requirements,
design, acceptance, and verification evidence, as well as repository docs,
tests, CI configuration, and implementation comments. Do not assume that
standalone requirements.md, design.md, or validation.md files exist: discover
the actual artifacts first, using the repository's native tools and Beads
commands when available.

The subagent is a read-only specification-drift reviewer. It must compare
user intent and accepted requirements against design, validation, tests, docs,
and implementation; identify omissions, contradictions, stale artifacts,
unverified claims, and scope creep; cite concrete files, Beads issue IDs,
symbols, tests, or line numbers; and distinguish confirmed drift from an
inference. It must not edit files, update Beads, commit, or propose a fix as if
it were already approved. Return a severity-ranked report with a concise
alignment verdict and actionable remediation suggestions.

The review should prioritize changed and recently completed work, configuration
and public-contract drift, security or data-integrity implications, and gaps
between acceptance criteria and executable verification. Preserve repository
instructions and do not read or modify external PromptKit source files.
