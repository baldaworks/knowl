...
becomes ready. `pkg/knowlfx.NewApp` and `pkg/knowlfx.Module` give Fx the host
lifecycle without re-implementing assembly: startup binds the loopback HTTP
listener and starts the bounded single-consumer writer; shutdown marks the host
unready, drains or interrupts queued work, recovers the workspace, and closes
the operational store. The worker follows the same non-blocking `Start`/`Stop`
shape used by the local lifecycle contract while preserving one-writer ordering
for canonical files.
