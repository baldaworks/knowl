...
func (store *Store) PrepareSync(ctx context.Context, prepared app.PreparedSyncState) (knowl.SyncRun, error) {
	...
	for _, document := range documents {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO knowl_sync_candidates (run_id, document_id, action, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted_at, candidate_digest) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			prepared.RunID, document.State.DocumentID, document.Action, document.State.Revision, accepted, document.State.MirrorPath, document.State.MirrorDigest, document.State.LastSeenRunID, optionalTime(document.State.DeletedAt), prepared.CandidateDigest)
		if err != nil {
...
func loadPreparedCandidateRows(ctx context.Context, queryer scanQueryer, run knowl.SyncRun) ([]app.PreparedDocumentState, error) {
	rows, err := queryer.QueryContext(ctx, `SELECT document_id, action, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted_at FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, run.ID)
	if err != nil {
	...
	for rows.Next() {
		var documentID, action, revision, accepted, mirrorPath, mirrorDigest, lastSeen, deletedAt string
		if err := rows.Scan(&documentID, &action, &revision, &accepted, &mirrorPath, &mirrorDigest, &lastSeen, &deletedAt); err != nil {
			return nil, fmt.Errorf("scan prepared candidate: %w", err)
		}
		state := knowl.DocumentState{Scope: run.Scope, SourceID: run.SourceID, DocumentID: knowl.DocumentID(documentID), Revision: revision, MirrorPath: mirrorPath, MirrorDigest: mirrorDigest, LastSeenRunID: knowl.SyncRunID(lastSeen), Deleted: action == string(app.SyncDocumentTombstone)}
...
func (store *Store) FinalizeSync(ctx context.Context, finalization app.SyncFinalization) (knowl.SyncRun, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT document_id, action, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted_at FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, finalization.RunID)
	if err != nil {
		return knowl.SyncRun{}, fmt.Errorf("read sync candidates: %w", err)
	}
	type candidate struct{ documentID, action, revision, accepted, mirrorPath, mirrorDigest, lastSeen, deletedAt string }
	var candidates []candidate
	for rows.Next() {
		var item candidate
		if err := rows.Scan(&item.documentID, &item.action, &item.revision, &item.accepted, &item.mirrorPath, &item.mirrorDigest, &item.lastSeen, &item.deletedAt); err != nil {
			_ = rows.Close()
	...
	for _, item := range candidates {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO knowl_source_documents (scope, source_id, document_id, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted, deleted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			ON CONFLICT(scope, source_id, document_id) DO UPDATE SET revision = excluded.revision, accepted_source = excluded.accepted_source, mirror_path = excluded.mirror_path, mirror_digest = excluded.mirror_digest, last_seen_run_id = excluded.last_seen_run_id, deleted = excluded.deleted, deleted_at = excluded.deleted_at, updated_at = excluded.updated_at`,
			finalization.Scope, finalization.SourceID, item.documentID, item.revision, item.accepted, item.mirrorPath, item.mirrorDigest, item.lastSeen, boolInt(deleted), item.deletedAt, formatTime(now), formatTime(now))
		if err != nil {
...
func (store *Store) DocumentState(ctx context.Context, scope knowl.ScopeRef, sourceID knowl.SourceID, documentID knowl.DocumentID) (knowl.DocumentState, error) {
	...
	row := store.db.QueryRowContext(ctx, `SELECT revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted, deleted_at, created_at, updated_at FROM knowl_source_documents WHERE scope = ? AND source_id = ? AND document_id = ?`, scope, sourceID, documentID)
...
func (store *Store) DocumentStates(ctx context.Context, scope knowl.ScopeRef, sourceID knowl.SourceID, options app.DocumentListOptions) ([]knowl.DocumentState, error) {
	...
	query := `SELECT document_id, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted, deleted_at, created_at, updated_at FROM knowl_source_documents WHERE scope = ? AND source_id = ?`
	...
	for rows.Next() {
		...
		var revision, accepted, mirrorPath, mirrorDigest, lastSeen, deletedAt, createdAt, updatedAt string
		var deleted int
		if err := rows.Scan(&documentID, &revision, &accepted, &mirrorPath, &mirrorDigest, &lastSeen, &deleted, &deletedAt, &createdAt, &updatedAt); err != nil {
			return nil, fmt.Errorf("scan source document: %w", err)
		}
		state, err := decodeDocumentState(scope, sourceID, documentID, revision, accepted, mirrorPath, mirrorDigest, lastSeen, deleted, deletedAt, createdAt, updatedAt)
		if err != nil {
			return nil, err
		}
...
func scanDocumentState(row scanner, scope knowl.ScopeRef, sourceID knowl.SourceID, documentID knowl.DocumentID) (knowl.DocumentState, error) {
	var revision, accepted, mirrorPath, mirrorDigest, lastSeen, deletedAt, createdAt, updatedAt string
	var deleted int
	err := row.Scan(&revision, &accepted, &mirrorPath, &mirrorDigest, &lastSeen, &deleted, &deletedAt, &createdAt, &updatedAt)
	if errors.Is(err, sql.ErrNoRows) {
	...
	return decodeDocumentState(scope, sourceID, documentID, revision, accepted, mirrorPath, mirrorDigest, lastSeen, deleted, deletedAt, createdAt, updatedAt)
}
func decodeDocumentState(scope knowl.ScopeRef, sourceID knowl.SourceID, documentID knowl.DocumentID, revision, accepted, mirrorPath, mirrorDigest, lastSeen string, deleted int, deletedAt, createdAt, updatedAt string) (knowl.DocumentState, error) {
	state := knowl.DocumentState{Scope: scope, SourceID: sourceID, DocumentID: documentID, Revision: revision, MirrorPath: mirrorPath, MirrorDigest: mirrorDigest, LastSeenRunID: knowl.SyncRunID(lastSeen), Deleted: deleted != 0}
	if err := json.Unmarshal([]byte(accepted), &state.AcceptedSource); err != nil {
...
func preparedDocumentsMatchTx(ctx context.Context, tx *sql.Tx, runID knowl.SyncRunID, documents []app.PreparedDocumentState, digest string) (bool, error) {
	rows, err := tx.QueryContext(ctx, `SELECT document_id, action, revision, accepted_source, mirror_path, mirror_digest, last_seen_run_id, deleted_at, candidate_digest FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, runID)
	if err != nil {
	...
	for rows.Next() {
		...
		var documentID, action, revision, accepted, mirrorPath, mirrorDigest, lastSeen, deletedAt, storedDigest string
		if err := rows.Scan(&documentID, &action, &revision, &accepted, &mirrorPath, &mirrorDigest, &lastSeen, &deletedAt, &storedDigest); err != nil {
		...
		if documentID != string(document.State.DocumentID) || action != string(document.Action) || revision != document.State.Revision || accepted != string(encoded) || mirrorPath != document.State.MirrorPath || mirrorDigest != document.State.MirrorDigest || lastSeen != string(document.State.LastSeenRunID) || deletedAt != optionalTime(document.State.DeletedAt) || storedDigest != digest {
			return false, nil
		}
		index++
	}
