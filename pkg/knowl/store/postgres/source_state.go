...
func (store *Store) PrepareSync(ctx context.Context, prepared app.PreparedSyncState) (knowl.SyncRun, error) {
	...
	for _, document := range documents {
		...
		_, err = sourceExec(ctx, tx, `INSERT INTO knowl_sync_candidates (run_id, document_id, action, revision, accepted_source, maintenance_revision, maintenance_operation_id, maintenance_generation, mirror_path, mirror_digest, last_seen_run_id, deleted_at, candidate_digest) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func loadPreparedCandidateRows(ctx context.Context, queryer sourceQueryer, run knowl.SyncRun) ([]app.PreparedDocumentState, error) {
	rows, err := sourceQuery(ctx, queryer, `SELECT document_id, action, revision, accepted_source, maintenance_revision, maintenance_operation_id, maintenance_generation, mirror_path, mirror_digest, last_seen_run_id, deleted_at FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, run.ID)
	if err != nil {
	...
	for rows.Next() {
		var documentID, action, revision, maintenanceRevision, maintenanceOperationID, maintenanceGeneration, mirrorPath, mirrorDigest, lastSeen string
		var accepted []byte
...
func (store *Store) FinalizeSync(ctx context.Context, finalization app.SyncFinalization) (knowl.SyncRun, error) {
	...
	rows, err := sourceQuery(ctx, tx, `SELECT document_id, action, revision, accepted_source, maintenance_revision, maintenance_operation_id, maintenance_generation, mirror_path, mirror_digest, last_seen_run_id, deleted_at FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, finalization.RunID)
	if err != nil {
	...
	type candidate struct {
		...
		maintenanceGeneration, mirrorPath, mirrorDigest, lastSeen                           string
	...
	for _, item := range candidates {
		...
		_, err = sourceExec(ctx, tx, `INSERT INTO knowl_source_documents (scope, source_id, document_id, revision, accepted_source, maintenance_revision, maintenance_operation_id, maintenance_generation, mirror_path, mirror_digest, last_seen_run_id, deleted, deleted_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
			...
			finalization.Scope, finalization.SourceID, item.documentID, item.revision, item.accepted, item.maintenanceRevision, item.maintenanceOperationID, item.maintenanceGeneration, item.mirrorPath, item.mirrorDigest, item.lastSeen, deleted, nullableTime(item.deletedAt), formatTime(now), formatTime(now))
		if err != nil {
...
func (store *Store) DocumentStates(ctx context.Context, scope knowl.ScopeRef, sourceID knowl.SourceID, options app.DocumentListOptions) ([]knowl.DocumentState, error) {
	...
	for rows.Next() {
		...
		var revision, accepted, maintenanceRevision, maintenanceOperationID, maintenanceGeneration, mirrorPath, mirrorDigest, lastSeen string
		var deletedAt sql.NullTime
...
func decodeDocumentState(scope knowl.ScopeRef, sourceID knowl.SourceID, documentID knowl.DocumentID, revision, accepted, maintenanceRevision, maintenanceOperationID, maintenanceGeneration, mirrorPath, mirrorDigest, lastSeen string, deleted bool, deletedAt sql.NullTime, createdAt, updatedAt time.Time) (knowl.DocumentState, error) {
	state := knowl.DocumentState{Scope: scope, SourceID: sourceID, DocumentID: documentID, Revision: revision, MaintenanceRevision: maintenanceRevision, MaintenanceOperationID: knowl.OperationID(maintenanceOperationID), MaintenanceGeneration: maintenanceGeneration, MirrorPath: mirrorPath, MirrorDigest: mirrorDigest, LastSeenRunID: knowl.SyncRunID(lastSeen), Deleted: deleted}
...
func preparedDocumentsMatchTx(ctx context.Context, tx *sql.Tx, runID knowl.SyncRunID, documents []app.PreparedDocumentState, digest string) (bool, error) {
	rows, err := sourceQuery(ctx, tx, `SELECT document_id, action, revision, accepted_source, maintenance_revision, maintenance_operation_id, maintenance_generation, mirror_path, mirror_digest, last_seen_run_id, deleted_at, candidate_digest FROM knowl_sync_candidates WHERE run_id = ? ORDER BY document_id`, runID)
	if err != nil {
	...
	for rows.Next() {
		...
		var documentID, action, revision, accepted, maintenanceRevision, maintenanceOperationID, maintenanceGeneration, mirrorPath, mirrorDigest, lastSeen, storedDigest string
		var deletedAt sql.NullTime
